   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         ++retval;
          ...
          std::cerr << "Warning: " << "in utf.h getUTF8StringLength: string '" << s << "' not encoded in unicode utf-8" << std::endl;
         return retval;
      }
   }
   if (idx != s.length()) {
...
inline int getCharactersFromUTF8String(const std::string &s, CSentence *sentence) {
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         idx += 3;
          ...
          std::cerr << "Warning: " << "in utf.h getCharactersFromUTF8String: string '" << s << "' not encoded in unicode utf-8" << std::endl;
         sentence->push_back("?");
         ++len;
         ++idx;
      }
   ...
   if (idx != s.length()) {
       std::cerr << "Warning: " << "in utf.h getCharactersFromUTF8String: string '" << s << "' not encoded in utf-8" << std::endl;
      return len;
   }
...
inline std::string getFirstCharFromUTF8String(const std::string &s) {
   ...
   if ((s[0]&0x80)==0) {
      ...
      std::cerr << "Warning: " << "in utf.h getFirstCharFromUTF8String: string '" << s << "' not encoded in unicode utf-8" << std::endl;
      return "?";
   }
}
   ...
   while (idx<s.length()) {
      if ((s[idx]&0x80)==0) {
         ...
         idx += 3;
          ...
          std::cerr << "Warning: " << "in utf.h getLastCharFromUTF8String: string '" << s << "' not encoded in unicode utf-8" << std::endl;
         return "?";
      }
   }
   if (idx != s.length()) {
