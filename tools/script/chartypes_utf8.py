...
dbc = {
        ...
        "punc-ext": [
            u"", u"Ⅺ", u"è", u"é", u"ì", u"í", u"à", u"ǔ", u"ú", u"ù",
            ...
            u"ｗ", u"ｐ", u"ｑ", u"ｒ", u"｜", u"｝", u"～", u"\\", u"ｘ", u"ｙ",
            u"ｚ", u"ｄ", u"ｅ", u"ｆ", u"ｇ", u"｀", u"ａ", u"ｂ", u"ｃ", u"ｌ",
            u"ｍ", u"ｎ", u"ｏ", u"ｈ", u"ｉ", u"ｊ", u"ｋ",]
};
...
def chartype(ch, encoding="utf-8"):
    ch = ch.decode(encoding)

    if ((ch in dbc["uppercase"]) or
            (ch in dbc["lowercase"]) or
            (ch in sbc["uppercase"]) or
            (ch in sbc["lowercase"])):
        return "LETTER"

    if ((ch in dbc["digit"]) or
            (ch in sbc["digit"])):
        return "DIGIT"

    if ((ch in dbc["punc"]) or
            (ch in dbc["chinese-punc"]) or
            (ch in dbc["punc-ext"]) or
            (ch in sbc["punc"])):
        return "PUNC"

    return "OTHER"

def sbc2dbc(ch, encoding="utf-8"):
    ch = ch.decode(encoding)

    if (ch in sbc["uppercase"]):
        return dbc["uppercase"][sbc["uppercase"].index(ch)]

    if (ch in sbc["lowercase"]):
        return dbc["lowercase"][sbc["lowercase"].index(ch)]

    if (ch in sbc["punc"]):
        return dbc["punc"][sbc["punc"].index(ch)]

    if (ch in sbc["digit"]):
        return dbc["digit"][sbc["digit"].index(ch)]
